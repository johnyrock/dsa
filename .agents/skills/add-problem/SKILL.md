...
### Narration voice standard

- The header introduction and every narration step from Step 2 onward use a natural first-person presenter voice: explain what **I** first consider, check, store, and conclude. Write as a person walking an audience through their own reasoning, not as a manual.
- Step 1, **The problem**, is the exception. Keep it neutral, simple, and beginner-friendly: state the input, requested output, concrete running example, and any important distinction such as indices versus values. Do not use first person there.
- Step 2 is always titled **First instinct** (a clarifying subtitle after a colon is fine). It must show a complete, correct baseline solution or algorithm for the stated constraints in a short `<pre class="code">` block. Do not present invalid code as a solution; if a tempting idea fails, discuss it later as a pitfall or contrast.
- Immediately after that baseline code, explain both its time and space complexity. The following paragraph must explain its concrete waste or limitation and how I can improve it. This sequence is required even when the baseline is already acceptable for small inputs.
- Outside **The problem**, paragraphs, callouts, and list items sound like a presenter explaining their own reasoning. Neutral factual sentences are fine, but do not use second-person or collective manual voice such as “you”, “your”, “we”, “our”, or “notice”. Do not use manual imperatives such as “Use…”, “Keep…”, “Push…”, or “Sort…” at the start of a narrative block, sentence, or explanatory clause (including “So/Then/Now push…”); state what I do instead.
- Use varied, problem-specific transitions. Do not repeat boilerplate such as “I can improve”, “This baseline is correct”, “My first correct baseline”, “I’ll start with the direct approach”, or “then improve it as I trace the running example”. State the concrete repeated work or constraint and the specific idea that replaces it.
...
6. Run the narration audit: `node .agents/skills/add-problem/scripts/audit-walkthrough-narration.js <tier>/<NNN>-<slug>/walkthrough.html`. It checks the structural requirements and flags deterministic narration issues outside **The problem**: manual second-person or collective voice, manual imperatives at paragraph, sentence, or clause boundaries, listed boilerplate transitions, and a few conservative awkward rewrite patterns. It strips `<pre>` and inline `<code>` content before checking, then reports the offending visible text.
